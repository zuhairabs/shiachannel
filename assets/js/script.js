(function ($) {
  ...
  var channelID = "UCb-4Mr8FtL34-v4mM3E8hiA";
  ...
  $(document).ready(function ($) {
    ...
    function renderTimings(timings, data) {
      ...
      var arWeekday = decodeURIComponent(
        JSON.parse('"' + dat.hijri.weekday.ar.replace(/\"/g, '\\"') + '"')
      );
      var arMonth = decodeURIComponent(
        JSON.parse('"' + dat.hijri.month.ar.replace(/\"/g, '\\"') + '"')
      );
      $("#hijrDate").html(
        `${arWeekday} ${dat.hijri.day} ${arMonth} ${dat.hijri.year}`
      );

      if (dat.hijri.holidays) {
        for (let h of dat.hijri.holidays) {
          $("#holidays").html(
            `<i class="ti-announcement" aria-hidden="true"></i> <span style="color:#44a884">${h}</span>`
          );
        }
      } else {
  ...
  $(document).ready(function ($) {
    if (localStorage.getItem("currentLanguage")) {
      ...
      localStorage.setItem(
        "currentLanguage",
        "https://shiachannel.in/assets/images/flags/america.png"
      );
    }
